...
const unsigned GCLL_HEAD = UINT_MAX - 1;
const unsigned GCLL_TAIL = UINT_MAX;
const unsigned GCLL_OK = 1;
const unsigned GCLL_ERROR = 0;
 ...
 * */
Cell *getCell(const LinkedList *list, int pos);
...
unsigned GCLL_Sort(LinkedList *list, int (*compare)(const void *, const void *)) {
    void **array = malloc(sizeof(void *) * GCLL_GetSize(list));
    if (!array)
        return GCLL_ERROR;
