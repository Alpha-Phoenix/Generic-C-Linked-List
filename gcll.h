...
#define GCLL_DEBUG // TODO remove in production

#include <limits.h>
#include <stddef.h>
#include <stdbool.h>

extern const int GCLL_HEAD;    ///< Flag used to insert/remove data from head (left)
extern const int GCLL_TAIL;    ///< Flag used to insert/remove data from tail (right)
extern const int GCLL_OK;      ///< Flag used to indicate operation success
extern const int GCLL_ERROR;   ///< Flag used to indicate operation failure
 ...
 * */
int GCLL_Push (LinkedList *list, int pos, void* data);
 ...
 * @return A pointer to the data stored in the requested position.
 * */
void* GCLL_Get (const LinkedList *list, int pos);
 ...
 * */
LinkedList* GCLL_Sort (const LinkedList *list, int (*compare) (const void *x, const void *y));
